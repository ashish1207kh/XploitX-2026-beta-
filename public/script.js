...
function initUrgencyTicker() {
    ...
    const messages = [
        ...
        "⚠️ CRITICAL: 24 HRS CAPTURE THE FLAG (CTF) Team slots filling up fast (85%)",
...
if (regForm) {
    // Configuration
    const EVENT_CONFIG = {
        '24 HRS CAPTURE THE FLAG (CTF)': { min: 2, max: 4, fee: 150, perHead: true }, // Day 1
    ...
    function init() {
        ...
        function renderEventOptions(day) {
            ...
            if (day === 'Day 1') {
                const opts = ['24 HRS CAPTURE THE FLAG (CTF)', 'Workshop'];
                opts.forEach(opt => {
        ...
        function handleSelectionChange() {
            ...
            if (day1Radios.length > 0) {
                const ctfChecked = document.querySelector('input[name="day1_event_radio"][value="24 HRS CAPTURE THE FLAG (CTF)"]:checked');
            ...
            const isCTF = selectedEvents.includes('24 HRS CAPTURE THE FLAG (CTF)');
            if (isCTF) {
            ...
            const isFreeAccess = selectedEvents.includes('paper_presentation') || selectedEvents.includes('24 HRS CAPTURE THE FLAG (CTF)');
        ...
        regForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const hasFreeAccess = selectedEvents.includes('paper_presentation') || selectedEvents.includes('24 HRS CAPTURE THE FLAG (CTF)');
    ...
    function restoreFromStorage() {
        ...
        if (eventCookie) {
            ...
            if (daySel && events.length > 0) {
                // Determine Day
                const day1Events = ['24 HRS CAPTURE THE FLAG (CTF)', 'Workshop'];
